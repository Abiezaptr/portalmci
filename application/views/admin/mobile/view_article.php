     ...
     <div class="container-fluid">
         ...
         <div class="card shadow mb-4">
             <div class="card-header py-3 d-flex justify-content-between align-items-center">
                 <!-- Title on the left -->
                 <h6 class="m-0 font-weight-bold" style="color: maroon;">Mobile Articles</h6>
                                     ...
                                     <tr>
                                         ...
                                         <td>
                                             <?php if (!empty($report['file'])): ?>
                                                 <a style="color: maroon;" href="<?= base_url('uploads/articles/mobile/' . $report['file']); ?>" target="_blank">Open</a>
                                             <?php else: ?>
                                                 No file available
                                             <?php endif; ?>
                                         </td>
                                         <td>
                                             <a href="#" class="btn btn-sm" class="btn btn-sm" data-toggle="modal" data-target="#updateArticleModal<?= $report['id']; ?>"><i class="fa fa-pencil-alt" style="color: maroon;"></i></a>&nbsp;
                                             <?php if ($this->session->userdata('role') == 1) : ?>
                                                 <a href="<?= site_url('delete-article/' . $report['id']); ?>" class="btn btn-sm"><i class="fa fa-trash-alt" style="color: maroon;"></i></a>
                                             <?php endif; ?>
                                         </td>
                                     </tr>
                                 <?php endforeach; ?>
     ...
     <div class="modal fade" id="addArticleModal" tabindex="-1" role="dialog" aria-labelledby="addArticleModalLabel" aria-hidden="true">
         <div class="modal-dialog modal-lg modal-dialog-centered" role="document"> <!-- Added modal-dialog-centered -->
             <div class="modal-content">
                 ...
                 <form id="categoryForm" action="<?= site_url('insert-article'); ?>" method="post" enctype="multipart/form-data">
     ...
     <?php foreach ($reports as $report) : ?>
         <div class="modal fade" id="updateArticleModal<?= $report['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="updateArticleModalLabel" aria-hidden="true">
             <div class="modal-dialog modal-lg modal-dialog-centered" role="document"> <!-- Added modal-dialog-centered -->
                 <div class="modal-content">
                     <div class="modal-header">
                         <h5 class="modal-title" id="updateArticleModalLabel">Update Articles</h5>
                         <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                             <span aria-hidden="true">&times;</span>
                         </button>
                     </div>
                     <form id="categoryForm<?= $report['id']; ?>" action="<?= site_url('update-articles/' . $report['id']); ?>" method="post" enctype="multipart/form-data">
                         <div class="modal-body">
                             <div class="row">
                                 <div class="col-md-12">
                                     <div class="form-group">
